  ...
  const processReceiptImage = async (dataUrl: string, sampleFallback?: SampleReceipt) => {
    ...
    try {
      ...
      if (sampleFallback) {
        ...
        setMerchant('Receipt Purchase');
        setNotes('Receipt attached on static mode');
        setScanError('Static hosting detected (e.g. GitHub Pages). Serverless OCR is unavailable, but you can enter the details below and your receipt photo will be saved.');
        setScanComplete(true);
      }
    } finally {
      setIsScanning(false);
